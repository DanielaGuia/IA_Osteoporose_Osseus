%CLASSIFICADORES ___ regressão logística
...
%Features relevantes estão ordenadas,são as primeiras 17 colunas e a 18 é o outcome
...
for i=1:10
    ...
    P=0.7;           % 70% dos dados vão para treino
    idx=randperm(m)
...
%     X_test(:,14)=(X_test(:,4)-mediar)/(stdr);
    ...
    for j=2:17
        ...
        xTest2=X_test(:,1:j);
        mdl=fitclinear(xTreino2,y_train,'Learner','logistic');
